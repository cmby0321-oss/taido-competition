...
function UpdateTableResult({ event_name, id, is_mobile, return_url }) {
    ...
    async function fetchData() {
      if (id !== undefined) {
        const response = await fetch(
          "/api/get_game_on_table?event_name=" + event_name + "&id=" + id,
        );
  ...
  const onSubmit = (data, values, initialValues, event_name) => {
    let post = {
      id: data.id,
    ...
    if (values[7] !== null && values[8] !== null) {
      post["sub1_score"] =
        (parseInt(values[7]) * 10 + parseInt(values[8])) / 10;
    }
    if (values[9] !== null && values[10] !== null) {
      post["sub2_score"] =
        (parseInt(values[9]) * 10 + parseInt(values[10])) / 10;
    }
      ...
      <Container maxWidth="md">
        ...
        <Box display="flex" alignItems="center" justifyContent="center">
          <Button
            variant="contained"
            type="submit"
            onClick={(e) => onSubmit(data, values, initialValues, event_name)}
          >
            決定
          </Button>
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
export default UpdateTableResult;
